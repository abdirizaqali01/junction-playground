'use client'

import { useState, useEffect } from 'react'
import { Footer } from "@/components/footer"

export default function MainChallengeDetailsPage() {
  const [sidebarState, setSidebarState] = useState('expanded') // 'expanded', 'icons', 'collapsed'
  const [lastClickTime, setLastClickTime] = useState(0)
  
  const handleSidebarToggle = () => {
    const currentTime = Date.now()
    const timeDiff = currentTime - lastClickTime
    
    if (timeDiff < 500) { // Double click within 500ms
      setSidebarState('collapsed')
    } else {
      if (sidebarState === 'expanded') {
        setSidebarState('icons')
      } else {
        setSidebarState('expanded')
      }
    }
    setLastClickTime(currentTime)
  }

  return (
    <div className="min-h-screen bg-black text-white flex">
      {/* Sidebar - Updated to match doc 1 colors */}
      <div className={`${
        sidebarState === 'collapsed' ? 'w-0' : 
        sidebarState === 'icons' ? 'w-16' : 'w-[200px]'
      } bg-white/10 border-r border-white/15 flex flex-col transition-all duration-300 fixed h-screen overflow-hidden pt-[1%]`}>
        {/* Back to Home Button */}
        <div className="p-4 border-b border-white/20 flex items-center justify-between">
          {sidebarState === 'expanded' && (
            <button className="flex items-center text-white/60 hover:text-white text-sm">
              <span className="mr-2">←</span>
              Back To Home
            </button>
          )}
          <button 
            onClick={handleSidebarToggle}
            className="p-1 text-white/60 hover:text-white ml-auto"
          >
            <svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
              <path d="M1.5 1.5A.5.5 0 0 1 2 1h12a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-.128.334L10 8.692V13.5a.5.5 0 0 1-.342.474l-3 1A.5.5 0 0 1 6 14.5V8.692L1.628 3.834A.5.5 0 0 1 1.5 3.5v-2z"/>
            </svg>
          </button>
        </div>

        {/* Image Placeholder */}
        {sidebarState === 'expanded' && (
          <div className="p-4">
            <div className="w-full h-32 bg-white/20 rounded-lg flex items-center justify-center">
              <div className="text-center">
                <div className="w-12 h-12 bg-white/30 rounded-full mx-auto mb-2"></div>
                <div className="w-16 h-8 bg-white/30 rounded mx-auto"></div>
              </div>
            </div>
          </div>
        )}

        {/* Navigation */}
        <nav className="flex-1 px-4 overflow-y-auto">
          <div className="mb-6">
            {sidebarState === 'expanded' && (
              <div className="text-xs text-white/40 uppercase tracking-wider mb-3 font-medium">General</div>
            )}
            
            <a href="#" className="flex items-center px-3 py-2 text-sm text-white/60 hover:text-white hover:bg-white/10 rounded mb-1" title="Dashboard">
              <svg className="w-4 h-4 mr-3" fill="currentColor" viewBox="0 0 16 16">
                <path d="M8 4a.5.5 0 0 1 .5.5V6a.5.5 0 0 1-1 0V4.5A.5.5 0 0 1 8 4zM3.732 5.732a.5.5 0 0 1 .707 0l.915.914a.5.5 0 1 1-.708.708l-.914-.915a.5.5 0 0 1 0-.707zM2 10a.5.5 0 0 1 .5-.5h1.586a.5.5 0 0 1 0 1H2.5A.5.5 0 0 1 2 10zm9.5 0a.5.5 0 0 1 .5-.5h1.586a.5.5 0 0 1 0 1H12a.5.5 0 0 1-.5-.5zm.754-4.246a.389.389 0 0 0-.527-.02L7.547 9.31a.91.91 0 1 0 1.302 1.258l3.434-4.297a.389.389 0 0 0-.029-.518z"/>
              </svg>
              {sidebarState === 'expanded' && "Dashboard"}
            </a>
            
            <a href="#" className="flex items-center px-3 py-2 text-sm text-white bg-white/20 rounded mb-1" title="Challenges">
              <svg className="w-4 h-4 mr-3" fill="currentColor" viewBox="0 0 16 16">
                <path d="M2.5 3A1.5 1.5 0 0 0 1 4.5v.793c.026.009.051.02.076.032L7.674 8.51c.206.1.446.1.652 0l6.598-3.185A.755.755 0 0 1 15 5.293V4.5A1.5 1.5 0 0 0 13.5 3h-11Z"/>
              </svg>
              {sidebarState === 'expanded' && "Challenges"}
            </a>
            
            <a href="#" className="flex items-center px-3 py-2 text-sm text-white/60 hover:text-white hover:bg-white/10 rounded mb-1" title="Team">
              <svg className="w-4 h-4 mr-3" fill="currentColor" viewBox="0 0 16 16">
                <path d="M7 14s-1 0-1-1 1-4 5-4 5 3 5 4-1 1-1 1H7zm4-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
                <path fillRule="evenodd" d="M5.216 14A2.238 2.238 0 0 1 5 13c0-1.355.68-2.75 1.936-3.72A6.325 6.325 0 0 0 5 9c-4 0-5 3-5 4s1 1 1 h4.216z"/>
              </svg>
              {sidebarState === 'expanded' && "Team"}
            </a>
            
            <a href="#" className="flex items-center px-3 py-2 text-sm text-white/60 hover:text-white hover:bg-white/10 rounded mb-1" title="Hackerpack">
              <svg className="w-4 h-4 mr-3" fill="currentColor" viewBox="0 0 16 16">
                <path d="M8 1a2.5 2.5 0 0 1 2.5 2.5V4h-5v-.5A2.5 2.5 0 0 1 8 1zm3.5 3v-.5a3.5 3.5 0 1 0-7 0V4H1v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V4h-3.5z"/>
              </svg>
              {sidebarState === 'expanded' && "Hackerpack"}
            </a>
          </div>

          <div className="mb-6">
            {sidebarState === 'expanded' && (
              <div className="text-xs text-white/40 uppercase tracking-wider mb-3 font-medium">During Hackathon</div>
            )}
            
            <a href="#" className="flex items-center px-3 py-2 text-sm text-white/60 hover:text-white hover:bg-white/10 rounded mb-1" title="Project Submission">
              <svg className="w-4 h-4 mr-3" fill="currentColor" viewBox="0 0 16 16">
                <path d="M14 1a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1H4.414A2 2 0 0 0 3 11.586l-2 2V2a1 1 0 0 1 1-1h12zM2 0a2 2 0 0 0-2 2v12.793a.5.5 0 0 0 .854.353l2.853-2.853A1 1 0 0 1 4.414 12H14a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2z"/>
              </svg>
              {sidebarState === 'expanded' && "Project Submission"}
            </a>
            
            <a href="#" className="flex items-center px-3 py-2 text-sm text-white/60 hover:text-white hover:bg-white/10 rounded mb-1" title="Mentor Meetings">
              <svg className="w-4 h-4 mr-3" fill="currentColor" viewBox="0 0 16 16">
                <path d="M6 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm-5 6s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H1zM11 3.5a.5.5 0 0 1 .5-.5h4a.5.5 0 0 1 0 1h-4a.5.5 0 0 1-.5-.5zm.5 2.5a.5.5 0 0 0 0 1h4a.5.5 0 0 0 0-1h-4zm2 3a.5.5 0 0 0 0 1h2a.5.5 0 0 0 0-1h-2z"/>
              </svg>
              {sidebarState === 'expanded' && "Mentor Meetings"}
            </a>
            
            <a href="#" className="flex items-center px-3 py-2 text-sm text-white/60 hover:text-white hover:bg-white/10 rounded mb-1" title="Review Projects">
              <svg className="w-4 h-4 mr-3" fill="currentColor" viewBox="0 0 16 16">
                <path d="M1.5 1.5A.5.5 0 0 1 2 1h12a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-.128.334L10 8.692V13.5a.5.5 0 0 1-.342.474l-3 1A.5.5 0 0 1 6 14.5V8.692L1.628 3.834A.5.5 0 0 1 1.5 3.5v-2z"/>
              </svg>
              {sidebarState === 'expanded' && "Review Projects"}
            </a>
            
            <a href="#" className="flex items-center px-3 py-2 text-sm text-white/60 hover:text-white hover:bg-white/10 rounded mb-1" title="Finalist Voting">
              <svg className="w-4 h-4 mr-3" fill="currentColor" viewBox="0 0 16 16">
                <path d="m8 2.748-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143c.06.055.119.112.176.171a3.12 3.12 0 0 1 .176-.17C12.72-3.042 23.333 4.867 8 15z"/>
              </svg>
              {sidebarState === 'expanded' && "Finalist Voting"}
            </a>
          </div>
        </nav>

        {/* User Profile at Bottom */}
        <div className="p-4 border-t border-white/20 mt-auto">
          <div className="flex items-center">
            <div className="w-12 h-12 bg-green-600 rounded-lg flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
              JU
            </div>
            {sidebarState === 'expanded' && (
              <div className="ml-3 min-w-0">
                <div className="text-sm font-medium text-white truncate">Junction Hack</div>
                <div className="text-xs text-white/60 truncate">ju@hackjunction.com</div>
              </div>
            )}
          </div>
        </div>
      </div>

      {/* Main Content */}
      <div className={`flex-1 overflow-auto flex flex-col transition-all duration-300 pt-[3%] ${
        sidebarState === 'collapsed' ? 'ml-0' : 
        sidebarState === 'icons' ? 'ml-16' : 'ml-[200px]'
      }`}>
        {/* Header */}
        <div className="bg-black border-b border-white/10 px-8 py-6">
          <button className="flex items-center text-white/60 hover:text-white text-sm mb-6">
            <span className="mr-2">←</span>
            Back To Challenges
          </button>
        </div>

        {/* Hero Banner Card with Rounded Corners */}
        <div className="px-8 pt-6">
          <div className="relative h-64 bg-gradient-to-r from-blue-500 via-purple-500 to-green-500 flex items-end overflow-hidden rounded-xl">
            {/* Blurred Background */}
            <div className="absolute inset-0 bg-gradient-to-r from-blue-500 via-purple-500 to-green-500 filter blur-sm"></div>
            <div className="absolute inset-0 bg-black/40"></div>
            
            {/* Text Content - Bottom Left */}
            <div className="relative z-10 p-6 pb-6">
              <div className="text-sm text-white/90 mb-2 font-mono">Partner Name</div>
              <h1 className="text-3xl font-bold text-white" style={{fontFamily: 'Space Grotesk, sans-serif'}}>
                Utilizing Generative Artificial Intelligence In A Sustainable Manner To Collect And Summarize Real Time Insights To Help Drive Business Decisions.
              </h1>
            </div>
          </div>
        </div>

        {/* Content */}
        <div className="flex-1 p-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Main Content */}
            <div className="lg:col-span-2 space-y-8">
              {/* The Challenge Section - Updated to match doc 1 colors */}
              <div className="bg-white/10 border border-white/15 rounded-xl p-8">
                <h2 className="text-2xl font-semibold text-white mb-6" style={{fontFamily: 'Space Grotesk, sans-serif'}}>
                  The Challenge
                </h2>
                
                <div className="space-y-4 text-base text-white/80 leading-relaxed font-mono">
                  <p>
                    Your challenge, should you accept it, is to build a sustainable AI assistant for dynamic information retrieval and summarization to 
                    improve customer office and also operational efficiency. We want you to use them to bring strong, effective, user-friendly AI tools to 
                    focus on two aspects of <span className="text-green-400">Sustainable AI: trustworthiness and efficiency</span> (which we are using here as a proxy for ecological 
                    sustainability through more limited energy use and carbon footprints). So this is not "just" about building an information retrieval 
                    assistant and feeding huge amounts of data to an enormous model – that is pretty too. We want you to build a system that accomplishes 
                    the same but in a very different way, and that makes things quite a bit more challenging – so you need to work 
                    smart not just hard on how you accomplish the task, so "trustworthy" this will not result in the desired outcome. But hey, that's why 
                    this is called a challenge; we know you are up for it!
                  </p>

                  <p>
                    Trustworthiness has also a few different aspects. First, it is about using the right data – as correct information as possible so the 
                    source. We all know that AI internet is not necessarily always correct... so how do you know what sources to trust? But even when 
                    you have the right information, summarizing that might not always get you what you want either. One of the fairly common 
                    challenges of generative models is the phenomenon of "effectiveness: getting results to questions where there are none. This leads to a lack 
                    of trust and can even be a barrier to usage. You should strive to build a system that bases it its outputs on sources you can trust and 
                    avoids situations to ensure the outputs of the system are reliable and trustworthy.
                  </p>

                  <p>
                    Furthermore, as we know, large language models are quite resource intensive. Especially training models tends to consume large 
                    amounts of energy, but the inference (when the Artificial Intelligence AI is used) can be costly – especially when done at scale. As it is not in 
                    scope for this challenge to build a model from scratch, let's focus on the computational costs related to using an existing model – 
                    both in terms of computational load to choose and how we use it. This is something we can often control better in practical applications 
                    too: as even if we use large, large models, we can definitely decide which ones to use and how we use them. So while this is a very 
                    context case of the large, resource intensive process is to generalize the system or the biggest context or just piles (tens or hundreds 
                    of queries to a model – it is the total cost that counts) – and a big part of the challenge is to optimize the resource usage while still 
                    getting great results.
                  </p>

                  <p>
                    This solution, if successful, can make a huge difference – and shed light on how tasks like can be accomplished in a sustainable way. 
                    So it is not just about building something very cool and useful but showcasing how thinking about all of the sustainability of AI can 
                    make a real positive difference!
                  </p>
                </div>
              </div>

              {/* Insight Section - Updated to match doc 1 colors */}
              <div className="bg-white/10 border border-white/15 rounded-xl p-8">
                <h2 className="text-2xl font-semibold text-white mb-6" style={{fontFamily: 'Space Grotesk, sans-serif'}}>
                  Insight
                </h2>
                
                <div className="space-y-4 text-base text-white/80 leading-relaxed font-mono">
                  <p>
                    As in real world situations we see a recurring different types of approaches, and ideally the solution could have these general 
                    approaches to building systems rather than considering the technical and domain-specific content. Three important 
                    key questions which we want you to think about and work on developing content to understand and that could appear to be:
                  </p>

                  <div className="ml-4 space-y-2">
                    <p>1. What is the expected development of business cloud market pricing for a specific group in the next month?</p>
                    <p>2. What are the three most recent analysis of energy policy time horizons and how do they integrate oil price.</p>
                    <p>3. What are the most interesting trends in the development of electric automotive markets 2025 recently?</p>
                  </div>

                  <p>
                    The questions show be such that the actual model component is easily interchangeable (so this is a quality developing focus 
                    and evolving from the assumption that you do not need to fine tune or retrain. The actual model and system the data with 
                    promising in a way that you get most effective that in order to evaluate your technological consumption you would tell you it also 
                    estimate how energy involved with the system having been used and understanding that this is just an estimate, thus using the number 
                    of tokens (or a simple approximation based on number of words and complexity) – it is part of what your solution will need to be doing 
                    anyways.
                  </p>

                  <p>
                    The architecture should be such that the actual model component is easily interchangeable (so this is a quality developing focus 
                    and evolving from the assumption that you do not need to fine tune or retrain). The actual model and system the data with 
                    promising in a way that you get most effective that in order to evaluate your technological consumption you would fill you it also 
                    estimate how energy involved with the system having been used and understanding that this is just an estimate, thus using the number 
                    of tokens (or a simple approximation based on number of words and complexity) – it is part of what your solution will need to be doing 
                    anyway.
                  </p>
                </div>
              </div>

              {/* What We'll Bring Section - Updated to match doc 1 colors */}
              <div className="bg-white/10 border border-white/15 rounded-xl p-8">
                <h2 className="text-2xl font-semibold text-white mb-6" style={{fontFamily: 'Space Grotesk, sans-serif'}}>
                  What We'll Bring
                </h2>
                
                <div className="space-y-4 text-base text-white/80 leading-relaxed font-mono">
                  <p>
                    The team and expert mentors VGAICO (Call WhatsApp) together technology. We do guide that the nature of the EU's 
                    focus lies on output systems it ensures game tech any rather Implementation initiative these technologies we help welcome. Given 
                    everything which gain plus we provide infrastructure, but this is not necessarily them plus providing expert technical advice: as it is an 
                    entirely technology focused matter, we can work together towards a common goal and the understanding that this is not a simple 
                    technological problem, but rather a rather complex set of applied research and implementation question.
                  </p>

                  <p>
                    Beyond this is a two-person project taking you know think you will spend external sources which more often like the you can hope that 
                    this try as used and Market technical progress.
                  </p>

                  <p>
                    VGAICO exposure as AV for understanding operation from technology and coordination, as a Graphical Processing Unit. Performed 
                    with other Research that focus on World Health Vision or ThereDB is not required, but if you have any experience with them, you'll 
                    bring value are all your power but VGAICO does not guess every time what your coding.
                  </p>
                </div>
              </div>
            </div>

            {/* Sidebar */}
            <div className="space-y-6">
              {/* Company Name Card - Updated to match doc 1 colors */}
              <div className="bg-white/10 border border-white/15 rounded-xl p-6">
                <h3 className="text-xl font-semibold text-white mb-4" style={{fontFamily: 'Space Grotesk, sans-serif'}}>
                  Company Name
                </h3>
                
                <div className="text-sm text-white/80 leading-relaxed font-mono">
                  <p className="mb-4">
                    QuickSight is a global leader in adaptive user UX, an 
                    evolving lifestyle brand that bring forth a unique 
                    niche for the producer of the most sustainable tech-base chain in 
                    the world. You can also build world-wide companies to reduce their 
                    climate impact through new sustainable solutions. Our core goal is to 
                    becoming in-Tech use new and big impact inquiry and learning.
                  </p>
                  
                  <p className="mb-4">
                    We are headquartered in Helsinki, Finland and originally since 
                    world. Our people are at the heart of our strategy, building 
                    trust and rapport, thus growing the employee long-term in 
                    career journey. Sustainability is central to what we do and the 
                    goals and vision. Always across building a free design and 
                    implementation, but much more importantly what the life is 
                    with is all around.
                  </p>
                  
                  <p>
                    Our AI and Digital Innovation Hub is a unit inside QuickSight. 
                    We are focused on defining new partnerships with AI-based 
                    platform for use cases ranging from energy optimization, to 
                    condition to health use-case. We welcome insight to the 
                    transformative business life-cycle process throughout AI 
                    projects and intelligent business as world's a strategic AI 
                    results.
                  </p>
                </div>
              </div>

              {/* Judging Criteria Card - Updated to match doc 1 colors */}
              <div className="bg-white/10 border border-white/15 rounded-xl p-6">
                <h3 className="text-xl font-semibold text-white mb-4" style={{fontFamily: 'Space Grotesk, sans-serif'}}>
                  Judging Criteria
                </h3>
                
                <div className="text-sm text-white/80 leading-relaxed font-mono">
                  <p className="mb-4">
                    The judging will be based on a combination of subjective 
                    evaluation of the technical implementation and quality of outputs 
                    by the QuickSight team. This also evaluates coherence over too 
                    long term.
                  </p>
                </div>

                <div className="space-y-4">
                  <div>
                    <h4 className="text-white font-semibold mb-2">Criteria 1</h4>
                    <p className="text-xs text-white/70 font-mono">
                      Truthful factors of Result – Publications expose the method 
                      from on a computational and a meaning that the project needs on 
                      evaluating the topic are the needed information your specific 
                      data and so that make of these sense but it is clear what it is 
                      trying to aim for or does not work with.
                    </p>
                  </div>

                  <div>
                    <h4 className="text-white font-semibold mb-2">Criteria 2</h4>
                    <p className="text-xs text-white/70 font-mono">
                      Efficiency of the solution – ideally the amount of computational 
                      systems that you are using energy consumption should be 
                      reasonable in relation to the level of sophistication and quality 
                      of resulting information. What some to getting computational most 
                      is rather they focus for being optimal resource efficiency, thus this.
                    </p>
                  </div>

                  <div>
                    <h4 className="text-white font-semibold mb-2">Criteria 3</h4>
                    <p className="text-xs text-white/70 font-mono">
                      Comprehensiveness and functionality of the solution – 
                      demonstrated through the implementation and GUI that the 
                      innovative project and of the the implementation can 
                      solution successfully and including the available solutions and the 
                      methods ideal benefit of a response is a 1-2 paragraphs, but this 
                      naturally dependent how responsive is to the kind.
                    </p>
                  </div>

                  <div>
                    <h4 className="text-white font-semibold mb-2">Criteria 4</h4>
                    <p className="text-xs text-white/70 font-mono">
                      Attention to detail – you should come towards a structure that 
                      clearly all the you should be able to understand good over 
                      resources and management, to give of the expected over the the 
                      integration. For example more ecological language models, 
                      designs.
                    </p>
                  </div>
                </div>
              </div>

              {/* Prizes Card - Updated to match the reference design */}
              <div className="bg-white/10 border border-white/15 rounded-xl p-6">
                <h3 className="text-xl font-semibold text-white mb-6" style={{fontFamily: 'Space Grotesk, sans-serif'}}>
                  Prizes
                </h3>
                
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center flex-shrink-0">
                      <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
                      </svg>
                    </div>
                    <div>
                      <div className="text-base font-medium text-white">1st Place</div>
                      <div className="text-sm text-white/60 font-mono">Winning team will get 5000€</div>
                    </div>
                  </div>

                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center flex-shrink-0">
                      <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
                      </svg>
                    </div>
                    <div>
                      <div className="text-base font-medium text-white">2nd Place</div>
                      <div className="text-sm text-white/60 font-mono">Team will receive 3000€</div>
                    </div>
                  </div>

                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center flex-shrink-0">
                      <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
                      </svg>
                    </div>
                    <div>
                      <div className="text-base font-medium text-white">3rd Place</div>
                      <div className="text-sm text-white/60 font-mono">Team will receive 1000€</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <Footer />
      </div>
    </div>
  )
}
